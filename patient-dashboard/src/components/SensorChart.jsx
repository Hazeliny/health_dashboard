...
const LegendBlock = () => (
  <div className="absolute right-2 flex flex-col items-start space-y-0 text-xs">
    <div className="flex items-center space-x-1"><div className="w-3 h-2 bg-red-500 rounded-sm" /><span className="dark:text-gray">Above Normal</span></div>
    <div className="flex items-center space-x-1"><div className="w-3 h-2 bg-yellow-300 rounded-sm" /><span className="dark:text-gray">Below Normal</span></div>
    <div className="flex items-center space-x-1"><div className="w-3 h-2 bg-blue-500 rounded-sm" /><span className="dark:text-gray">Normal</span></div>
  </div>
)
...
//    console.log("🔍 Fetching trend data from:", url); 
    ...
    console.log("🔍 Fetching trend data from:", `${API_BASE}/api/trend-data?metric=${metricKey}&patientId=${patientId}&range=${range}`); 
        ...
        if (isBloodPressure) {
          ...
          return (
            <div key={index} 
              ...
              >
              {/* Title */}
              <h4 className="text-lg font-semibold mb-0 whitespace-pre-line dark:text-gray">{card.title}</h4>
          ...
          return (
            <div key={index} className="group p-4 border rounded shadow-sm relative cursor-pointer hover:shadow-lg hover:border-blue-600 transition" onClick={() => handleCardClick(card.key)}
              title="Click to view visual monitor"
            >
              <h4 className="text-lg font-semibold mb-4 whitespace-pre-line dark:text-grey">{card.title}</h4>
